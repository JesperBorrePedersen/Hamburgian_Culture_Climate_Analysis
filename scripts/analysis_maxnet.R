...
# Set sf without s2 (this script was written before s2 was adapted by sf)
sf_use_s2(FALSE)
...
fit_maxnet <- function(training_data, absence_data, k){
    # Status
    cat("Fitting maxnet model with", k, "-fold cross validation.\n" )
    ...
    for(i in 1:5){
      ...
      # Fit maxnet model to training data
      maxnet_models[[i]] <- maxnet(training_group %>% 
                                   pull(presence),
                                 training_group %>%
                                   select(mean_temp,
                                          mean_precip))
            # Evaluate model
      maxnet_evals[[i]] <- evaluate(p = training_group %>% 
                                    filter(presence  == 1),
                                  a = training_group %>%  
                                    filter(presence == 0), 
                                   model = maxnet_models[[i]],
                                  type = "cloglog")
    }
    ...
    mean_tss <- mean(unlist(lapply(maxnet_evals, 
                                   function(x) {
                                     thresh <- threshold(x)["spec_sens"][1,1,]
    ...
    # fit single model 
    maxnet_model <- maxnet(bind_rows(training_data,
                                     absence_data) %>% 
                             pull(presence),
                           bind_rows(training_data,
                             ...
                             select(mean_temp,
                                    mean_precip))
    maxnet_eval <- evaluate(p = bind_rows(training_data,
                                          absence_data) %>%
                              filter(presence == 1),
    ...
    maxnet_thresh <-  threshold(maxnet_eval, 'spec_sens') 
    
    # Extract AUC, Kappa, TSS
    auc <- maxnet_eval@auc
    kappa <-  maxnet_eval@kappa[which(maxnet_eval@t == maxnet_thresh)]
    tss <-  maxnet_eval@TPR[which(maxnet_eval@t == maxnet_thresh)] + maxnet_eval@TNR[which(maxnet_eval@t == maxnet_thresh)] - 1
    
    # Status
    cat("Done.\n")
    
    # return all outputs
    list(maxnet_models = maxnet_models,
         maxnet_evals = maxnet_evals,
         mean_auc = mean_auc,
         mean_kappa = mean_kappa,
         mean_tss = mean_tss,
         maxnet_model = maxnet_model,
         maxnet_eval = maxnet_eval,
         maxnet_thresh = maxnet_thresh,
         auc = auc,
         kappa = kappa,
         tss = tss)
}
...
# Global model
model_global <- fit_maxnet(bioclim_training_all,
                           absence_data,
                           5)
# Pulse 1
model_pulse_1 <- fit_maxnet(bioclim_training_all %>% 
...
# Pulse 2
model_pulse_2 <- fit_maxnet(bioclim_training_all %>% 
...
write_csv(eval_summary, "tables/supplementary/model_evaluation_maxent.csv")

# Predict for core time periods
predictions_global <- predict(climate_mean,
                              model_global[["maxnet_model"]],
                              type="cloglog")
predictions_pulse_1 <- predict(climate_pulse_1,
                                   model_pulse_1[["maxnet_model"]],
                               type="cloglog") 
predictions_pulse_2 <- predict(climate_pulse_2, 
                                   model_pulse_2[["maxnet_model"]],
                               type="cloglog")

# Apply threshold
predictions_global_thresh <- predictions_global > model_global[["maxnet_thresh"]]
predictions_pulse_1_thresh <- predictions_pulse_1 > model_pulse_1[["maxnet_thresh"]]
predictions_pulse_2_thresh <- predictions_pulse_2 > model_pulse_2[["maxnet_thresh"]]
...
# Export to file
png("figures/helper_figures/maxnet_predictions_scale.png", 
    width = 1.5,
    height = 3,
    units = "in",
    res = 300,
...
# Cut out scale
predictions_scale <- image_read("figures/helper_figures/maxnet_predictions_scale.png")
predictions_scale <- image_crop(predictions_scale, "450x180+0+480")
image_write(predictions_scale, "figures/helper_figures/maxnet_predictions_scale.png") 
...
# Export to file
png("figures/helper_figures/maxnet_predictions_legend.png", 
    width = 5,
    height = 3/6*5,
    units = "in",
    res = 300,
...
# Cut out legend
predictions_legend <- image_read("figures/helper_figures/maxnet_predictions_legend.png")
predictions_legend <- image_crop(predictions_legend, "263x152+1211+70")
image_write(predictions_legend, "figures/helper_figures/maxnet_predictions_legend.png") 
...
predict_time_series <- function(maxnet_model_list, model_name, file_name){
  # Status
  cat(paste0("Starting ", model_name, " Model ...\n"))
  
  # create folder
  out_folder <- paste0("figures/supplementary/maxnet_preds_ts_", file_name)
  dir.create(out_folder)
  
  # Calculate predictions
  cat(paste0("Calculating predictions ... \n"))
  maxnet_model <- maxnet_model_list[["maxnet_model"]]
  maxnet_thresh <- maxnet_model_list[["maxnet_thresh"]]
  ...
  preds_time_series <- lapply(time_series_climate, function(climate_raster){
    ...
    predictions <- predict(climate_raster,
                           ...
                           type = "cloglog")
    # Apply threshold
    predictions_thresh <- predictions > maxnet_thresh
...
# |_ Figure S1 - Suitabillity Predictions ----
# List of plots 
...
predictions_legend_panel <- plot_grid(add_title(predictions_scale, 
                                         "Predicted Value (maxnet, cloglog)",
                                         "plain",
                                         8,
                                         rel_height = 0.2,
                                         top_margin = 2,
...
save_plot("figures/supplementary/figure_S1_suitability_predictions_maxnet.png", 
          figure_s1,
          base_height = 2,
          ncol = 1,
          nrow = 3,
          base_asp = 1.85,
          bg = "white")
save_plot("figures/supplementary/figure_S1_suitability_predictions_maxnet.eps", 
          figure_s1,
          base_height = 2,
          ncol = 1,
          nrow = 3,
...
# |_ Figure S2 - Suitabillity Time-Series ----
# read in time-series plots (centuries 14.7-14.1)
ts_maps_global <- lapply(rev(list.files("figures/supplementary/maxnet_preds_ts_global/", pattern = ".png", full.names = T)[11:17]),
                         function(x) ggdraw() + draw_image(image_read(x), scale = 0.95))
ts_maps_pulse_1 <- lapply(rev(list.files("figures/supplementary/maxnet_preds_ts_pulse_1/", pattern = ".png", full.names = T)[11:17]),
                          function(x) ggdraw() + draw_image(image_read(x), scale = 0.95))
ts_maps_pulse_2 <- lapply(rev(list.files("figures/supplementary/maxnet_preds_ts_pulse_2/", pattern = ".png", full.names = T)[11:17]),
                          function(x) ggdraw() + draw_image(image_read(x), scale = 0.95))
...
# Assemble plot
figure_s2_body <- plot_grid(ts_time_scale,
                      ts_maps_global,
                      ts_maps_pulse_1,
                      ts_maps_pulse_2,
                      ncol = 4,
...
figure_s2_labels <- plot_grid(
  NULL, figure_s2_labels[[1]],
  figure_s2_labels[[2]], 
  figure_s2_labels[[3]],
  rel_widths =c(0.3,1,1,1),
  nrow= 1)
figure_s2_legend <- plot_grid(
  NULL, predictions_legend_panel, NULL,
  rel_widths = c(0.3,1.8,1.2),
  nrow = 1
)
figure_s2 <- plot_grid(figure_s2_labels,
                      figure_s2_body,
                      figure_s2_legend,
                      ncol = 1,
                      rel_heights = c(0.015,1,0.1))
save_plot("figures/supplementary/figure_S2-suitability_time_series_maxnet.png",
          figure_s2,
          base_height = 1.9,
          ncol = 3,
          nrow = 7,
          base_asp = 2,
          bg = "white")
save_plot("figures/supplementary/figure_S2-suitability_time_series_maxnet.eps",
          figure_s2,
          base_height = 1.9,
          ncol = 3,
          nrow = 7,
...
animate_time_series <- function(ts_name){
  ...
  # set folder
  out_folder <- paste0("figures/supplementary/maxnet_preds_ts_", ts_name)
  ...
  prediction_legend <- image_read("figures/helper_figures/maxnet_predictions_legend.png")
  ...
  image_write_gif(image = img_joined,
            path = paste0(out_folder, "/maxnet_preds_time_series_", ts_name, ".gif"),
            delay = 0.5)
  cat("\nDone.")
  return(NULL)
}
